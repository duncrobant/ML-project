...
ddos_model = RandomForestClassifier(n_estimators=10, random_state=42)
accuracy_list = []

def run_ddos_detect(display_log= True, display_graphs= True ):
    print("Starting: File Read")
    data = pd.read_csv('datasets/embedded_system_network_security_dataset.csv')
...
def train(data):
    ...
    fwd_pkts = data["Tot Fwd Pkts"]
    bwd_pkts = data["Tot Bwd Pkts"]
    pkts = data["Flow Pkts/s"]
    byts = data["Flow Byts/s"]
    fwd_len_mean = data["Fwd Pkt Len Mean"]
    syn_cnt = data["SYN Flag Cnt"]
